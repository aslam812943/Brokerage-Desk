  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        // Subsequent requests: re-verify against DB every SESSION_RECHECK_INTERVAL
        // This ensures deleted users / role changes propagate within 15 minutes
        const now = Date.now();
        if (!token.lastChecked || now - token.lastChecked > SESSION_RECHECK_INTERVAL) {
